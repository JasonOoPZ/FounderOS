...
const nextSteps = [
  ...
  { step: "3", title: "Then tackle AWS and GCP", body: "Have your funding docs ready. Worth $450K combined - do not skip these." },
];
...
export default function SuccessPage() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Inter, sans-serif" }}>
      <nav style={{ borderBottom: `1px solid ${C.border}`, height: "58px", display: "flex", alignItems: "center", padding: "0 48px" }}>
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto", padding: "80px 48px", textAlign: "center" }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <h1 style={{ fontSize: "clamp(32px, 5vw, 48px)", fontWeight: 900, color: C.ink, letterSpacing: "-2px", lineHeight: 1.05, marginBottom: "16px" }}>You are in.</h1>
          <p style={{ fontSize: "16px", color: C.mid, lineHeight: 1.7, marginBottom: "8px" }}>Welcome to Founder OS Pro.</p>
          {email && <p style={{ fontSize: "14px", color: C.light, marginBottom: "48px" }}>Access granted to <strong style={{ color: C.ink }}>{email}</strong></p>}
                ...
                <div key={item.step} style={{ display: "flex", gap: "16px", alignItems: "flex-start", background: C.surface, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "18px 20px" }}>
                  ...
                  <div>
                    <div style={{ fontSize: "14px", fontWeight: 700, color: C.ink, marginBottom: "4px" }}>
                      {(item as any).href ? <Link href={(item as any).href} style={{ color: C.ink, textDecoration: "none" }}>{item.title} →</Link> : item.title}
                    </div>
